package com.ssi;

import java.util.ArrayList;
import java.util.List;

import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.Transaction;

public class CascadeTwo {
	public static void main(String[] args) {

		Employee e1=new Employee(1,"AAA",new Laptop(1001,"Dell",51000));
		Employee e2=new Employee(2,"BBB",new Laptop(1002, "HP", 61000));
		Employee e3=new Employee(3,"CCC",new Laptop(1003, "Sony",71000));
		
		Project p1=new Project(555,"Project-1");
		Project p2=new Project(666,"Project-2");
		Project p3=new Project(777,"Project-3");
		Project p4=new Project(888,"Project-4");
		
		List<Project> list1=new ArrayList<>(); list1.add(p1); list1.add(p2);
		List<Project> list2=new ArrayList<>(); list2.add(p1); list2.add(p3); list2.add(p4);
		//e1.setProjects(list1);
		//e2.setProjects(list2);
		SessionFactory sf=DataUtility.getSessionFactory();
		Session session=sf.openSession();
		Transaction tr=session.beginTransaction();
		//session.save(e1); session.save(e2); session.save(e3);//not for cascade
		session.persist(e1); session.persist(e2); 
		//session.delete(session.get(Employee.class, 3));
		//session.delete(session.get(Laptop.class, 1003));

		tr.commit();
		session.close();
		System.out.println("EMPLOYEES ADDED");
	}

}
